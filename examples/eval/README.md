...
- **frontier-API baseline (optional):** `make spark-eval-frontier` or
 `FRONTIER=auto` — calls Anthropic only if
 `SPARK_EVAL_FRONTIER_API_KEY` / `ANTHROPIC_API_KEY` /
 `SPARK_EVAL_FRONTIER_KEY_FILE` is already set.
