...
def lint_jsonl(path: Path) -> list[str]:
    ...
    for i, line in enumerate(raw.splitlines(), 1):
        ...
        if not isinstance(obj, dict):
            ...
            continue
        # Common train pair shapes (chat messages or flat pairs)
        has_pair = False
        ...
        if not has_pair:
            errors.append(
                "line %d: need messages[] or user/assistant pair"
                % i
            )
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="spark-helper-fixture-lint")
    p.add_argument(
        "paths",
        nargs="+",
        help="JSONL fixture paths",
    )
