...
| **Builder from SPARK_BC** | `--compile` Spark → `.sparkbc` (incl. `0x26`/`0x28`/`0x27`); dump; emit init weights; bootstrap `--run-bc` dry train | Spark-created **init**, not trained; GAS emit/`--run-bc` **BLOCKED**; STEP→weights in flight; later train aims to beat Claude; [SPARK_BUILDER.md](SPARK_BUILDER.md) |
