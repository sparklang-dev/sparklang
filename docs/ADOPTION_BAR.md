...
| SPARK_BC Builder factory | **done (init)** | Spark → `--compile` → `.sparkbc` with `TRAIN`/`STEP`/`TRAIN_STATUS`; bootstrap `--run-bc` dry fixture; Spark-created init safetensors. Dry ≠ trained. GAS emit / `--run-bc` **BLOCKED**. STEP→weights **in flight**. Later train aims to beat Claude — not claimed today. Page: [SPARK_BUILDER.md](SPARK_BUILDER.md) / `/docs/spark-builder.html` |
